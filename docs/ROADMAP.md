...
- **`cuidSchema`-validate the rest of `discovery/actions.ts`** [low/S] — `cancelScan`, `approveDiscovery`, `assignToExistingDevice`, `rejectDiscovery` take a `scanId: string` straight to Prisma without the `cuidSchema` guard the convention requires (`deleteScan`/`deleteBuildPlan` now do). Pre-existing; sweep the file. *(convention_outlier)*
